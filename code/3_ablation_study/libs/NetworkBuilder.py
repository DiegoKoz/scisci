...
class NetworkBuilder:
    ...
    def remove_feature(self,metadata_df, remove):
        """
            remove: str, options: {"affiliation", "first_author", "coverDate", "subject_area", "topic_dist", "bert"} default None
        """
        if remove is not None:
            metadata_df = metadata_df.loc[:,~metadata_df.columns.str.startswith(remove)]
        return metadata_df
    
    def prepare_metadata(self, remove = None):
        """
            remove: str, options: {"affiliation", "first_author", "coverDate", "subject_area", "topic_dist", "bert", "citations_at"} default None
        """
        metadata_df = self.metadata.loc[:,['eid','affiliation','authors','subject_areas','coverDate','topic_dist','bert_embedding']]
        ...
        metadata_df = self.remove_feature(metadata_df,remove)
